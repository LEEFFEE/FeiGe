...
import cn.leeffee.feige.ui.cloud.model.FragGroupModelImpl;
import cn.leeffee.feige.ui.cloud.presenter.FragGroupPresenterImpl;
 ...
 */
public class GroupFragment extends BaseFragment<FragGroupPresenterImpl, FragGroupModelImpl> implements FragGroupContract.View, AdapterView.OnItemClickListener {
    ...
    private ServiceConnection mDownloadConn = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            Log.d("MyGroupDownloadConn", "onServiceConnected() called");
        ...
        public void onServiceDisconnected(ComponentName name) {
            Log.d("MyGroupDownloadConn", "onServiceDisconnected() called");
    ...
    private ServiceConnection mUploadConn = new ServiceConnection() {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
            Log.d("MyGroupUploadConn", "onServiceConnected() called");
        ...
        @Override
        public void onServiceDisconnected(ComponentName name) {
            Log.d("MyGroupUploadConn", "onServiceDisconnected() called");
            mIsUploadBind = false;
    ...
    protected int getLayoutResource() {
        return R.layout.frag_tab_group;
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        if (o instanceof USpaceGroup) {
            ...
            menu.findItem(R.id.menu_group_copy_to_my_space).setVisible(true);
            ...
            if (o instanceof USpaceGroup) {
                ...
                switch (item.getItemId()) {
                    case R.id.menu_group_download:
                        downloadFile(file);
                        break;
                    case R.id.menu_group_copy_to_my_space:
     ...
     * @param ufs 要复制的文件数组
     */
    private void openDirChoiceDialog(final USpaceFile[] ufs) {
        final ChoiceRemoteDictionaryAlertDialog dialog = new ChoiceRemoteDictionaryAlertDialog(getActivity());
        dialog.setPrefix("复制到:" + getText(R.string.tab_cloud));
        dialog.setOkClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mRxManager.add(mPresenter.copy2PrivateSpace(ufs, dialog.getDestPath(), current_group_id, REQUEST_CODE_COPY_TO_PRIVATE_SPACE));
                dialog.dismiss();
            }
        });
        dialog.show();
    ...
    public void loadBefore(String requestCode) {
        switch (requestCode) {
            ...
            case REQUEST_CODE_REMOVE:
                mLoadingLayout.setVisibility(View.VISIBLE);
                break;
            //            case REQUEST_CODE_LISTDIRONLYDIR_REMOTE:
    ...
    public void loadSuccess(String requestCode, Object result) {
        List<USpaceGroup> resGroups;
        List<USpaceFile> resFiles;
        switch (requestCode) {
            case REQUEST_CODE_FIND_SHARE_GROUP_BY_USER_INIT:
                adapter = new GroupListAdapter(getContext());
                mListView.setAdapter(adapter);
                resGroups = (List<USpaceGroup>) result;
            ...
            case REQUEST_CODE_LIST_GROUP_FILE_INIT:
                ...
                mListView.setAdapter(adapter);
                resFiles = (List<USpaceFile>) result;
