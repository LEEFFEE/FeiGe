...
public class OkHttpUtil {
    ...
    private OkHttpUtil() {
         ...
         */
        mClient.newBuilder().connectTimeout(5, TimeUnit.SECONDS);
        mClient.newBuilder().readTimeout(5, TimeUnit.SECONDS);
        mClient.newBuilder().writeTimeout(5, TimeUnit.SECONDS);
    ...
    public <T> T getSync(String url) {
        String json = getSyncString(url);
        //通过获取到的实例来调用内部方法
        T t = GsonUtil.gson.fromJson(json, new TypeToken<T>() {
        }.getType());
        return t;
    }
    ...
    public String getSyncString(String url) {
        return mOkHttpUtil.inner_getSyncString(url);
    }
     ...
     * @return
     */
    public String postSyncString(String url, String json) {
        return mOkHttpUtil.inner_postSyncString(url, json);
    }
     ...
     * 同步方法
     */
    private String inner_postSyncString(String url, String json) {
        String result = null;
        try {
            /**
             * 把取得到的结果转为字符串，这里最好用string()
